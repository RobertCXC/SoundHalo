...
- 顶部居中显示蓝牙音频设备或 COM 串口连接提示。
- 圆形提示平滑展开为蓝灰色玻璃质感的信息卡片。
- 显示设备名称和“已连接”状态。
- 使用透明宿主窗口，不激活窗口、不抢键盘焦点。
- 常驻系统托盘，不创建传统主窗口。
- 支持从托盘菜单或双击托盘图标测试弹窗动画。
- 支持从托盘菜单独立开关蓝牙监听和 COM 监听。
- 自动取消上一次提示，避免多个弹窗重叠。
- 蓝牙监听继续兼容常见的蓝牙耳机和蓝牙音频输出设备。
- 监听配置按用户保存在 `%LocalAppData%\SoundHalo\settings.json`，不污染程序目录。
...
程序启动时会先读取 `%LocalAppData%\SoundHalo\settings.json` 中的用户配置，只启动已启用的监听器。蓝牙监听仍然会建立一次 Windows 当前处于活动状态的音频输出端点快照，并注册 Windows Core Audio 的设备端点通知回调。端点新增、移除、状态或属性发生变化时，程序会在短暂防抖后重新枚举设备，通过设备标识、枚举器信息和设备名称判断其是否为蓝牙音频设备。COM 监听则会创建一个隐藏消息窗口，通过 `RegisterDeviceNotification` 接收 `WM_DEVICECHANGE` 设备通知，再枚举当前串口列表并仅对新增的 COM 口显示提示。两类监听都会在启动时先建立快照，只对启动后新增的设备显示提示，避免程序启动时重复弹窗。
...
- Win32 `RegisterDeviceNotification` / `WM_DEVICECHANGE`：监听 COM 串口设备变更
...
1. 启动程序后，应用会隐藏在系统托盘中，并按上次保存的设置启用蓝牙监听和/或 COM 监听。
2. 双击托盘图标，或在右键菜单中选择“测试弹窗”，预览提示动画。
3. 在托盘右键菜单中勾选或取消“监听蓝牙”“监听 COM”，修改会立即生效并自动保存。
4. 连接新的蓝牙音频设备或新增 COM 串口时，程序会自动显示设备名称和连接状态。
5. 在托盘右键菜单中选择“退出”关闭程序。
...
├─ ComPort/
│  └─ ComPortMonitor.cs                    COM 设备事件监听与枚举
├─ Popup/
...
├─ Settings/
│  ├─ AppSettings.cs                       用户监控配置模型
│  └─ AppSettingsStore.cs                  `%LocalAppData%` 配置读写
├─ Tray/
│  └─ TrayService.cs                       系统托盘图标、右键菜单和监控开关
├─ Native/
...
- 蓝牙仍只监控 Windows 音频输出端点；普通蓝牙键盘、鼠标等设备不会触发提示。
- COM 监听依赖 Windows 设备通知和串口枚举；仅对系统暴露为 COM 端口的设备显示提示。
- 设备检测采用 Windows 音频端点事件通知和 Win32 设备变更通知；收到通知后会按需刷新，不使用固定周期轮询。
